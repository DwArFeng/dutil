...
public class BitUtilTest {
    ...
    @Test
    public void testAnd_lengthMismatchMessageUsesScopedLocale() {
        IllegalArgumentException chinese = MessageContext.call(
                Locale.SIMPLIFIED_CHINESE,
                () -> assertThrows(
                        IllegalArgumentException.class,
                        () -> BitUtil.and(new byte[1], new byte[2], new byte[3])
                )
        );

        assertEquals("a、b、dest 长度必须相同：a=1，b=2，dest=3", chinese.getMessage());
    }
