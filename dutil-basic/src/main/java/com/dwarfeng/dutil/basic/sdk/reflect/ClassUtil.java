package com.dwarfeng.dutil.basic.sdk.reflect;

import com.dwarfeng.dutil.basic.internal.i18n.BasicMessageKey;
import com.dwarfeng.dutil.basic.internal.i18n.BasicMessages;

import java.util.*;

/**
 * 类功能包。
 *
 * <p>
 * 该包封装了一些 {@link Class} 包中没有的功能，比如列出一个包的所有直到 {@link Object} 的所有父类。
 *
 * @author DwArFeng
 * @since 0.0.2-beta
 */
public final class ClassUtil {

    /**
     * 获取一个类的所有父类。
     *
     * @param cl 指定的类。
     * @return 该类的所有父类组成的集合。
     * @throws NullPointerException 入口参数为 <code>null</code>。
     */
    public static Collection<Class<?>> getSuperClasses(Class<?> cl) {
        Objects.requireNonNull(cl, BasicMessages.message(BasicMessageKey.CLASS_TYPE_REQUIRED));
        Collection<Class<?>> collection = new HashSet<>();
        Class<?> clas = cl.getSuperclass();
        while (Objects.nonNull(clas)) {
            collection.add(clas);
            clas = clas.getSuperclass();
        }
        return collection;
    }

    /**
     * 获取一个类的所有父类。
     *
     * @param o 指定的实例。
     * @return 该类的所有父类组成的集合。
     * @throws NullPointerException 入口参数为 <code>null</code>。
     */
    public static Collection<Class<?>> getSuperClasses(Object o) {
        Objects.requireNonNull(o, BasicMessages.message(BasicMessageKey.CLASS_OBJECT_REQUIRED));
        return getSuperClasses(o.getClass());
    }

    /**
     * 获取一个实例的类中的所有父类。
     *
     * @param o 指定的实例。
     * @return 该实例的所有父类组成的数组。
     * @throws NullPointerException 入口参数为 <code>null</code>。
     */
    public static Class<?>[] getSuperClassArray(Object o) {
        Objects.requireNonNull(o, BasicMessages.message(BasicMessageKey.CLASS_OBJECT_REQUIRED));
        return getSuperClasses(o).toArray(new Class<?>[0]);
    }

    /**
     * 获取一个类中的所有父类。
     *
     * @param cl 指定的类。
     * @return 该类的所有父类组成的数组。
     * @throws NullPointerException 入口参数为 <code>null</code>。
     */
    public static Class<?>[] getSuperClassArray(Class<?> cl) {
        Objects.requireNonNull(cl, BasicMessages.message(BasicMessageKey.CLASS_TYPE_REQUIRED));
        return getSuperClasses(cl).toArray(new Class<?>[0]);
    }

    /**
     * 获取一个类所实现的所有接口，包括自身以及父类实现的所有接口和实现接口的父接口。
     *
     * @param cl 指定的类。
     * @return 该类以及父类实现的所有接口组成的集合。
     * @throws NullPointerException 入口参数为 <code>null</code>。
     */
    public static Collection<Class<?>> getImplInterfaces(Class<?> cl) {
        Objects.requireNonNull(cl, BasicMessages.message(BasicMessageKey.CLASS_TYPE_REQUIRED));

        // 将 cl 的所有接口加入到待处理集合中。
        Set<Class<?>> classInterfaceSet = new HashSet<>(Arrays.asList(cl.getInterfaces()));

        Set<Class<?>> superInterfaceSet = new HashSet<>();

        for (Class<?> clas : getSuperClasses(cl)) {
            classInterfaceSet.addAll(Arrays.asList(clas.getInterfaces()));
        }

        while (!classInterfaceSet.isEmpty()) {
            Class<?> clas = classInterfaceSet.iterator().next();
            classInterfaceSet.remove(clas);

            if (clas.isInterface()) {
                superInterfaceSet.add(clas);
            }
            classInterfaceSet.addAll(Arrays.asList(clas.getInterfaces()));
        }

        return superInterfaceSet;
    }

    /**
     * 获取一个实例的类所实现的所有接口，包括自身以及父类实现的所有接口和实现接口的父接口。
     *
     * @param o 指定的实例。
     * @return 该实例的类以及父类实现的所有接口组成的集合。
     * @throws NullPointerException 入口参数为 <code>null</code>。
     */
    public static Collection<Class<?>> getImplInterfaces(Object o) {
        Objects.requireNonNull(o, BasicMessages.message(BasicMessageKey.CLASS_OBJECT_REQUIRED));
        return getImplInterfaces(o.getClass());
    }

    /**
     * 获取一个类所实现的所有接口，包括自身以及父类实现的所有接口和实现接口的父接口。
     *
     * @param cl 指定的类。
     * @return 该类以及父类实现的所有接口组成的数组。
     * @throws NullPointerException 入口参数为 <code>null</code>。
     */
    public static Class<?>[] getImplInterfacesArray(Class<?> cl) {
        Objects.requireNonNull(cl, BasicMessages.message(BasicMessageKey.CLASS_TYPE_REQUIRED));
        return getImplInterfaces(cl).toArray(new Class<?>[0]);
    }

    /**
     * 获取一个实例的类所实现的所有接口，包括自身以及父类实现的所有接口和实现接口的父接口。
     *
     * @param o 指定的实例。
     * @return 该实例的类以及父类实现的所有接口组成的数组。
     * @throws NullPointerException 入口参数为 <code>null</code>。
     */
    public static Class<?>[] getImplInterfacesArray(Object o) {
        Objects.requireNonNull(o, BasicMessages.message(BasicMessageKey.CLASS_OBJECT_REQUIRED));
        return getImplInterfaces(o).toArray(new Class<?>[0]);
    }

    /**
     * 获取指定类的包装类。
     *
     * <p>
     * 如果指定的类是基础类型的话，则返回其包装类型；否则，则返回其自身。
     *
     * @param cl 指定的类。
     * @return 指定的类的包装类。
     * @throws NullPointerException     入口参数为 <code>null</code>。
     * @throws IllegalArgumentException 无法识别的基础类。
     */
    public static Class<?> getPackedClass(Class<?> cl) throws NullPointerException, IllegalArgumentException {
        Objects.requireNonNull(cl, BasicMessages.message(BasicMessageKey.CLASS_TYPE_REQUIRED));

        if (!cl.isPrimitive()) {
            return cl;
        }

        if (cl == byte.class) {
            return Byte.class;
        } else if (cl == short.class) {
            return Short.class;
        } else if (cl == int.class) {
            return Integer.class;
        } else if (cl == long.class) {
            return Long.class;
        } else if (cl == float.class) {
            return Float.class;
        } else if (cl == double.class) {
            return Double.class;
        } else if (cl == char.class) {
            return Character.class;
        } else if (cl == boolean.class) {
            return Boolean.class;
        } else if (cl == void.class) {
            return Void.class;
        }

        throw new IllegalArgumentException(BasicMessages.message(BasicMessageKey.CLASS_PRIMITIVE_TYPE_UNRECOGNIZED));
    }

    private ClassUtil() {
        throw new IllegalStateException(BasicMessages.message(BasicMessageKey.CLASS_INSTANTIATION_FORBIDDEN));
    }
}
