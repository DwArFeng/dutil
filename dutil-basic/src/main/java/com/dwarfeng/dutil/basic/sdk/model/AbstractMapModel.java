...
import com.dwarfeng.dutil.basic.stack.model.MapModel;
...
public abstract class AbstractMapModel<K, V> implements MapModel<K, V> {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractMapModel.class);
    ...
    public AbstractMapModel(Set<MapObserver<K, V>> observers) {
        Objects.requireNonNull(observers, BasicMessages.message(BasicMessageKey.ABSTRACT_MAP_MODEL_OBSERVERS_REQUIRED));
        ...
        for (MapObserver<K, V> observer : observers) {
            if (Objects.nonNull(observer))
                try {
                    ...
                    logObserverException("firePut", e);
                }
        }
    }
        ...
        for (MapObserver<K, V> observer : observers) {
            if (Objects.nonNull(observer))
                try {
                    ...
                    logObserverException("fireChanged", e);
                }
        }
    }
