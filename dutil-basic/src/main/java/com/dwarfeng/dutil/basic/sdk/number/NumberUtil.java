...
import com.dwarfeng.dutil.basic.stack.number.Interval;
import com.dwarfeng.dutil.basic.stack.number.NumberValue;
import com.dwarfeng.dutil.basic.stack.number.OutOfIntervalException;

import com.dwarfeng.dutil.basic.stack.number.unit.Angle;
...
public final class NumberUtil {
    ...
    public static int bytesToInt(byte[] bytes) {
        ByteBuffer buffer = ByteBuffer.allocate(Integer.BYTES);
        buffer.put(resizeBytes(bytes, Integer.BYTES)).flip();
    ...
    public static long bytesToLong(byte[] bytes) {
        ByteBuffer buffer = ByteBuffer.allocate(Long.BYTES);
        buffer.put(resizeBytes(bytes, Long.BYTES)).flip();
    ...
    public static float bytesToFloat(byte[] bytes) {
        ByteBuffer buffer = ByteBuffer.allocate(Float.BYTES);
        buffer.put(resizeBytes(bytes, Float.BYTES)).flip();
    ...
    public static double bytesToDouble(byte[] bytes) {
        ByteBuffer buffer = ByteBuffer.allocate(Double.BYTES);
        buffer.put(resizeBytes(bytes, Double.BYTES)).flip();
    ...
    public static short bytesToShort(byte[] bytes) {
        ByteBuffer buffer = ByteBuffer.allocate(Short.BYTES);
        buffer.put(resizeBytes(bytes, Short.BYTES)).flip();
     ...
     * @return 裁剪后的数组。
     */
    private static byte[] resizeBytes(byte[] bytes, int length) {
    ...
    public static int max(int... is) {
        Objects.requireNonNull(is, BasicMessages.message(BasicMessageKey.NUMBER_INT_VALUES_REQUIRED));
        if (is.length == 0)
            return 0;
        if (is.length == 1)
            return is[0];
    ...
    public static double max(double... ds) {
        Objects.requireNonNull(ds, BasicMessages.message(BasicMessageKey.NUMBER_DOUBLE_VALUES_REQUIRED));
        if (ds.length == 0)
            return 0;
        if (ds.length == 1)
            return ds[0];
    ...
    public static boolean isInInterval(int value, Interval interval) {
        Objects.requireNonNull(interval, BasicMessages.message(BasicMessageKey.NUMBER_INTERVAL_REQUIRED));
    ...
    private NumberUtil() {
        throw new IllegalStateException(BasicMessages.message(BasicMessageKey.NUMBER_INSTANTIATION_FORBIDDEN));
    }
}
