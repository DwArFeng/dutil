 ...
 *
 * <p>
 * 该面板的布局类似于边界布局样式中的布局，不过边界与边界之前有拖动条。通过拖动这些拖动条
...
public class JAdjustableBorderPanel extends JPanel {
    ...
    /**
     * 水平模板：默认显示东、中、西三个方向的空间，所有控件最小度量 100
     */
    public final static int MOUDLE_HOR = 1;
    /**
     * 竖直模板：默认显示北，中，南三个方向的控件，所有控件最小度量 100
     */
    public final static int MOUDLE_VEC = 2;
    /**
     * 中下模板：默认显示中，南两个方向的控件，所有控件最小度量 100
     */
    public final static int MOUDLE_CD = 3;
    /**
     * 全向模板：默认显示所有控件，最小度量 100
     */
     ...
     *
     * <p>
     * 该方法重写了父类方法，使其可以通过不同的约束向不同方的可拖动控件中添加指定的组组件。
     * <br>约束条件只能是{@linkplain BorderLayout} 的约束条件中的一个，否则会抛出异常。
     *
     ...
     * @throws IllegalArgumentException 当约束不等于{@code BorderLayout} 的任何一个约束时抛出该异常。
     */
    @Override
    ...
    public void setSeperatorThickness(int seperatorThickness) {
        ...
        if (northSeparator != null)
            northSeparator.setPreferredSize(new Dimension(this.seperatorThickness, this.seperatorThickness));
        if (southSeparator != null)
            southSeparator.setPreferredSize(new Dimension(this.seperatorThickness, this.seperatorThickness));
        if (westSeparator != null)
            westSeparator.setPreferredSize(new Dimension(this.seperatorThickness, this.seperatorThickness));
        if (eastSeparator != null)
            eastSeparator.setPreferredSize(new Dimension(this.seperatorThickness, this.seperatorThickness));
        if (northPan != null) northPan.repaint();
        if (southPan != null) southPan.repaint();
        if (eastPan != null) eastPan.repaint();
        if (westPan != null) westPan.repaint();
