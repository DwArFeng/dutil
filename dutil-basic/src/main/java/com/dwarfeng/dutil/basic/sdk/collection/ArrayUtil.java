...
public final class ArrayUtil {
    ...
    public static <T> T[] empty(Class<T> clas) throws NullPointerException {
        Objects.requireNonNull(clas, BasicMessages.message(BasicMessageKey.ARRAY_COMPONENT_TYPE_REQUIRED));
    ...
    public static <T> T[] getNonNull(T[] array) throws NullPointerException {
        Objects.requireNonNull(array, BasicMessages.message(BasicMessageKey.ARRAY_REQUIRED));
    ...
    public static <T> T[] concat(T[] first, T[] second) {
        Objects.requireNonNull(first, BasicMessages.message(BasicMessageKey.ARRAY_FIRST_REQUIRED));
        Objects.requireNonNull(second, BasicMessages.message(BasicMessageKey.ARRAY_SECOND_REQUIRED));
    ...
    public static byte[] unpack(Byte[] target) {
        ...
        if (target == null)
            throw new NullPointerException(BasicMessages.message(BasicMessageKey.COLLECTION_TARGET_REQUIRED));
    ...
    public static void requireInBounds(Object[] objs, int index) {
        Objects.checkIndex(index, Objects.requireNonNull(objs, "objs").length);
    }
    ...
    public static <T> Iterable<T> array2Iterable(T[] array) {
        Objects.requireNonNull(array, BasicMessages.message(BasicMessageKey.ARRAY_TARGET_REQUIRED));
    ...
    public static <T> T[] readOnlyArray(T[] array, ReadOnlyGenerator<T> generator) {
        Objects.requireNonNull(array, BasicMessages.message(BasicMessageKey.ARRAY_REQUIRED));
        Objects.requireNonNull(generator, BasicMessages.message(BasicMessageKey.ARRAY_GENERATOR_REQUIRED));

        @SuppressWarnings("unchecked")
        T[] targetArray = (T[]) Array.newInstance(array.getClass().getComponentType(), array.length);
    ...
    private ArrayUtil() {
        throw new IllegalStateException(BasicMessages.message(BasicMessageKey.ARRAY_INSTANTIATION_FORBIDDEN));
    }
}
