...
public class AttributeComplex {
    ...
    public static AttributeComplex newInstance(Object[] objects)
        ...
        Objects.requireNonNull(objects, BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_OBJECTS_REQUIRED));

        // 确保数组中元素的个数是偶数。
        if (objects.length % 2 != 0) {
            throw new IllegalArgumentException(
                    BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_OBJECT_COUNT_NOT_EVEN)
            );
        }
        ...
        for (int i = 0; i < objects.length; i += 2) {
            ...
            if (Objects.isNull(keyObject)) {
                throw new IllegalArgumentException(
                        BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_ODD_ELEMENT_REQUIRED)
                );
            }
            String key;
            if (keyObject instanceof String) {
                ...
                key = ((Name) keyObject).name();
            } else {
                throw new IllegalArgumentException(
                        BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_ODD_ELEMENT_TYPE_INVALID)
                );
            }
    ...
    protected AttributeComplex(Map<String, Object> delegate) throws NullPointerException, IllegalArgumentException {
        Objects.requireNonNull(delegate, BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_DELEGATE_REQUIRED));
        if (delegate.containsKey(null)) {
            throw new IllegalArgumentException(
                    BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_DELEGATE_CONTAINS_NULL)
            );
        }
    ...
    public boolean containsKey(String key) throws NullPointerException {
        Objects.requireNonNull(key, BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_KEY_REQUIRED));
    ...
    public Object get(String key) throws NullPointerException, IllegalArgumentException {
        Objects.requireNonNull(key, BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_KEY_REQUIRED));
        if (!containsKey(key)) {
            throw new IllegalArgumentException(BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_KEY_NOT_FOUND));
        }
    ...
    public <T> T get(String key, Class<T> clas)
        ...
        Objects.requireNonNull(key, BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_KEY_REQUIRED));
        Objects.requireNonNull(clas, BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_VALUE_TYPE_REQUIRED));
        if (!containsKey(key)) {
            throw new IllegalArgumentException(BasicMessages.message(BasicMessageKey.ATTRIBUTE_COMPLEX_KEY_NOT_FOUND));
        }
    ...
    public boolean equals(Object o) {
        if (o == this)
            return true;
        if (!(o instanceof AttributeComplex that))
            return false;
