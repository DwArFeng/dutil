...
public class TaskMessagesTest {
    ...
    public void testCatalogResourcesAndLocalizedMessages() throws IOException {
        for (TaskMessages.Catalog catalog : TaskMessages.Catalog.values()) {
            ...
            Set<String> expectedKeys = Arrays.stream(TaskMessageKey.values())
                    .filter(key -> key.catalog() == catalog)
                    .map(TaskMessageKey::key)
            ...
            for (TaskMessageKey key : TaskMessageKey.values()) {
                ...
                assertEquals(
                        ...
                        TaskMessages.message(Locale.ROOT, key, FORMAT_ARGUMENTS),
                        key.name() + " root"
                );
                assertEquals(
                        format(chinesePattern, Locale.SIMPLIFIED_CHINESE),
                        TaskMessages.message(Locale.SIMPLIFIED_CHINESE, key, FORMAT_ARGUMENTS),
                        key.name() + " zh_CN"
                );
            }
        }
    ...
    private static Map<String, String> loadMessages(TaskMessages.Catalog catalog, Locale locale)
            throws IOException {
    ...
    private static String resourceName(TaskMessages.Catalog catalog, Locale locale) {
